...
type Settings = {
  rewardConversionRate: string;
  invoicePrefix: string;
  billPrefix: string;
  gstEnabled: boolean;
  allowManualGstEdit: boolean;
...
const SCANNER_KEY = "my_scanner_config";
const SCANNER_TESTED_KEY = "my_scanner_tested_at";
...
const defaultSettings: Settings = {
  ...
  compactBillMode: false,
  rewardRedemptionEnabled: true,
  creditLimitDefault: "5000",
  overdueAlertDays: "30",
  enableCash: true,
  enableCard: true,
  enableUpi: true,
...
const inputCls =
  "w-full px-3 py-2 rounded-lg text-sm focus:outline-none text-slate-800 placeholder-slate-400" +
  " bg-slate-50 border border-slate-200 focus:bg-white focus:border-blue-400";
const selectCls =
...
export default function SettingsPage() {
      ...
      try {
        ...
        if (profRes.data.profile) {
          ...
          setProfile({
            name: p.name || "",
            address: p.address || "",
            storeMobile: p.storeMobile || "",
            storeEmail: p.storeEmail || "",
            storeWebsite: p.storeWebsite || "",
            state: p.state || "",
            pincode: p.pincode || "",
            placeOfSupply: p.placeOfSupply || "",
            gstNumber: p.gstNumber || "",
            logoUrl: p.logoUrl || "",
            loginImageUrl: p.loginImageUrl || "",
        ...
        if (rulesRes.data.settings) {
          ...
          setSettings({
            ...
            allowManualGstEdit: s.allowManualGstEdit ?? false,
            ...
            thermalHeaderText: s.thermalHeaderText || "",
            autoPrintAfterCheckout: s.autoPrintAfterCheckout ?? false,
            showLogoOnBill: s.showLogoOnBill ?? true,
        ...
        if (scannerStored) {
          try {
            setScannerConfig(JSON.parse(scannerStored));
          } catch {
            /* ignore malformed */
          }
        }
  ...
  const saveP = (id: string) =>
    run(id, () => apiClient.put("/settings/profile", profilePayload()));
  const saveS = (id: string) =>
    run(id, () => apiClient.put("/settings/rules", settingsPayload()));
  const savePS = (id: string) =>
    run(id, async () => {
  ...
  const handleTestPrint = () => {
    ...
    localStorage.setItem("my_thermal_tested_at", now);
  ...
  const getThermalStatus = () => {
    ...
    if (!settings.thermalPrinterName)
      return { label: "NOT CONFIGURED", cls: "bg-amber-100 text-amber-700" };
    if (thermalTestedAt) {
      const mins = Math.round(
        (Date.now() - new Date(thermalTestedAt).getTime()) / 60000,
      );
      const label =
        mins < 60
          ? `TESTED ${mins}m ago`
          : `TESTED ${Math.round(mins / 60)}h ago`;
  ...
  const getScannerStatus = () => {
    if (!scannerConfig.enabled)
      return { label: "DISABLED", cls: "bg-slate-100 text-slate-500" };
    if (!scannerConfig.name)
      return { label: "NOT CONFIGURED", cls: "bg-amber-100 text-amber-700" };
    if (scannerTestedAt) {
      const mins = Math.round(
        (Date.now() - new Date(scannerTestedAt).getTime()) / 60000,
      );
      const label =
        mins < 60
          ? `TESTED ${mins}m ago`
    ...
    <>
      ...
      <div className="dark-app flex flex-col min-h-screen print:hidden">
        ...
        <main className="flex-1 flex flex-col overflow-hidden">
          <header
            className="px-8 h-14 flex items-center shrink-0"
          ...
          <div className="flex-1 overflow-y-auto">
            <div className="max-w-3xl mx-auto px-8 py-8 space-y-6">
              ...
              <SectionCard
                title="Store Profile"
                subtitle="Basic information shown on bills and invoices"
              >
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <Field label="Mobile Number">
                    <input
                      className={inputCls}
                      type="tel"
                      value={profile.storeMobile}
                      onChange={(e) =>
                        setProfile((p) => ({
                          ...p,
                          storeMobile: e.target.value,
                        }))
                      }
                ...
                <SaveBtn
                  saving={isSaving("store-profile")}
                  onSave={() => saveP("store-profile")}
                />
              </SectionCard>
              ...
              <SectionCard
                title="Tax & GST"
                subtitle="GST registration and tax calculation rules"
              >
                <div className="space-y-4">
                  ...
                  <Field
                    ...
                    hint="Determines CGST+SGST (intrastate) vs IGST (interstate)"
                  >
                    <select
                      className={selectCls}
                      value={profile.placeOfSupply}
                      onChange={(e) =>
                        setProfile((p) => ({
                          ...p,
                  ...
                  />
                  <Toggle
                    checked={settings.allowManualGstEdit}
                    onChange={(v) =>
                      setSettings((s) => ({ ...s, allowManualGstEdit: v }))
                    }
                ...
                <SaveBtn
                  saving={isSaving("tax-gst")}
                  onSave={() => savePS("tax-gst")}
                />
              </SectionCard>
              ...
              <SectionCard
                title="Branding"
                subtitle="Logo, login page, and document appearance"
              >
                <div className="space-y-5">
                  ...
                  <div className="space-y-2">
                    <Toggle
                      checked={settings.showLogoOnBill}
                      onChange={(v) =>
                        setSettings((s) => ({ ...s, showLogoOnBill: v }))
                      }
                    ...
                    <Toggle
                      ...
                      onChange={(v) =>
                        setSettings((s) => ({ ...s, showLogoOnInvoice: v }))
                      }
                      label="Show Logo on Tax Invoice"
                    />
                    <Toggle
                      checked={settings.compactBillMode}
                      onChange={(v) =>
                  ...
                  >
                    <textarea
                      className={inputCls}
                      rows={2}
                      value={profile.invoiceFooterNote}
                      onChange={(e) =>
                        setProfile((p) => ({
                          ...p,
                ...
                <SaveBtn
                  saving={isSaving("branding")}
                  onSave={() => savePS("branding")}
                />
              </SectionCard>
              ...
              <SectionCard
                ...
                subtitle="Prefixes for tax invoice and thermal bill numbers"
              >
                ...
                <SaveBtn
                  saving={isSaving("numbering")}
                  onSave={() => saveS("numbering")}
                />
              </SectionCard>
              ...
              <SectionCard
                ...
                subtitle="Point earning rate, redemption, and credit defaults"
              >
                <div className="space-y-4">
                  <Field
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        onChange={(e) =>
                          setSettings((s) => ({
                            ...s,
                            rewardConversionRate: e.target.value,
                          }))
                        }
                        placeholder="0.1"
                      />
                  ...
                  <Toggle
                    checked={settings.rewardRedemptionEnabled}
                    onChange={(v) =>
                      setSettings((s) => ({ ...s, rewardRedemptionEnabled: v }))
                    }
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    >
                      <input
                        ...
                        onChange={(e) =>
                          setSettings((s) => ({
                            ...s,
                            creditLimitDefault: e.target.value,
                          }))
                        }
                        placeholder="5000"
                      />
                    ...
                    <Field
                      label="Overdue Alert (days)"
                      hint="Flag credit accounts overdue beyond this many days"
                    >
                      <input
                        className={inputCls}
                        type="number"
                        ...
                        onChange={(e) =>
                          setSettings((s) => ({
                            ...s,
                            overdueAlertDays: e.target.value,
                          }))
                        }
                        placeholder="30"
                      />
                ...
                <SaveBtn
                  saving={isSaving("rewards")}
                  onSave={() => saveS("rewards")}
                />
              </SectionCard>
              ...
              <SectionCard
                ...
                subtitle="Toggle which modes appear at checkout"
              >
                <div className="space-y-2">
                  <Toggle
                    checked={settings.enableCash}
                    onChange={(v) =>
                      setSettings((s) => ({ ...s, enableCash: v }))
                    }
                    label="Cash"
                  />
                  <Toggle
              ...
              <SectionCard
                ...
                subtitle="Receipt printing configuration"
              >
                <div className="flex items-center gap-3 mb-5">
                  ...
                  <span
                    className={`text-xs font-bold px-2.5 py-1 rounded-full ${thermal.cls}`}
                  >
                    {thermal.label}
                  </span>
                </div>
                <div className="space-y-4">
                  <Toggle
                    checked={settings.thermalPrintEnabled}
                    onChange={(v) =>
                      setSettings((s) => ({ ...s, thermalPrintEnabled: v }))
                    }
                    ...
                    <Field label="Paper Width">
                      <select
                        ...
                        onChange={(e) =>
                          setSettings((s) => ({
                            ...s,
                            thermalPaperWidth: e.target.value,
                          }))
                        }
                      >
                    ...
                    >
                      <input
                        ...
                        onChange={(e) =>
                          setSettings((s) => ({
                            ...s,
                            thermalPrinterName: e.target.value,
                          }))
                        }
                  ...
                  >
                    <input
                      className={inputCls}
                      value={settings.thermalHeaderText}
                      onChange={(e) =>
                        setSettings((s) => ({
                          ...s,
                  ...
                  <Toggle
                    ...
                    onChange={(v) =>
                      setSettings((s) => ({ ...s, autoPrintAfterCheckout: v }))
                    }
              ...
              <SectionCard
                ...
                subtitle="USB or Bluetooth keyboard-wedge barcode reader settings"
              >
                <div className="flex items-center gap-3 mb-5">
                  <span className="text-sm font-semibold text-slate-600">
                    Status:
                  ...
                  <span
                    className={`text-xs font-bold px-2.5 py-1 rounded-full ${scanner.cls}`}
                  >
                    {scanner.label}
                  </span>
                </div>
                ...
                <div className="space-y-4">
                  <Toggle
                    ...
                    onChange={(v) =>
                      setScannerConfig((c) => ({ ...c, enabled: v }))
                    }
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <Field
                      label="Scan Suffix"
                      hint="Character the scanner sends after each barcode"
                    >
                      <select
                        className={selectCls}
                        ...
                        onChange={(e) =>
                          setScannerConfig((c) => ({
                            ...c,
                            suffix: e.target.value as ScannerConfig["suffix"],
                          }))
                        }
                      >
                  ...
                  >
                    <svg
                      className="w-4 h-4 shrink-0 mt-0.5 text-sky-400"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                      strokeWidth={2}
                    ...
                    <div className="text-xs text-slate-500 leading-relaxed">
                      <span className="text-slate-700 font-semibold block mb-0.5">
                        How scanner integration works
                      </span>
                      USB and Bluetooth barcode scanners act as keyboard
                  ...
                  <Field
                    label="Test Scan"
                    hint="Click the field below, then scan any barcode to verify your scanner is working"
                  >
                ...
                >
                  <p className="text-xs text-slate-400">
                    Scanner settings are saved to this device only.
                  </p>
                  <button
                    type="button"
                    onClick={saveScanner}
